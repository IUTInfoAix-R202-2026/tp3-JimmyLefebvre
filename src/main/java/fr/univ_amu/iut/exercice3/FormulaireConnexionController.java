...
public class FormulaireConnexionController {
  ...
  private void initialize() {
    ...
    // 1. Le champ mot de passe n'est éditable que si l'identifiant >= 6 caractères
    champMotDePasse
        .editableProperty()
        .bind(Bindings.greaterThanOrEqual(champIdentifiant.textProperty().length(), 6));

    // 2. Annuler est désactivé si les deux champs sont vides
    boutonAnnuler
        .disableProperty()
        .bind(
            Bindings.and(
    ...
    // 3. OK est désactivé tant que le mot de passe n'est pas valide
    BooleanBinding motDePasseInvalide =
        new BooleanBinding() {
          ...
          protected boolean computeValue() {
            ...
            boolean sansMajuscule = mdp.chars().noneMatch(Character::isUpperCase);
            boolean sansChiffre = mdp.chars().noneMatch(Character::isDigit);
            return tropCourt || sansMajuscule || sansChiffre;
          }
        };
